...
(ns ^{:skip-wiki true}
  ...
  (:require [clojure.core.async.impl.protocols :as impl]
            ...
            [clojure.core.async.impl.platform.linkedlist :refer [next remove hasNext isEmpty iterator clear size add] :as linked-list]
  ...
  (:import 
   ...
   [clojure.core.async.impl.platform.linkedlist LinkedList]
...
(defn- handle [buf exh t]
  (let [else ((or exh (platform/default-exception-handler)) t)]
    (if (nil? else)
      buf
      (impl/add! buf else))))
...
(defn chan
  ...
  ([buf xform exh]
     (ManyToManyChannel.
      (linked-list/linked-list) (linked-list/linked-list) buf (atom false) (lock/mutex)
